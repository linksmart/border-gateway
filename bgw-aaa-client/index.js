...
const ps_log_level = log_levels[config.log_level[0].toUpperCase()] - 1;
const can_log = (cat) => log_levels[cat[0]] > ps_log_level;
const timestamp = () => `[${new Date().toLocaleString()}]`;

const logFunction = {
    log: (cat, ...arg) => can_log(cat) && console.log(cat, ...arg),
    logTS: (cat, ...arg) => can_log(cat) && console.log(timestamp(), cat, ...arg)
};

const isTS = (!config.no_timestamp) ? "TS" : "";
