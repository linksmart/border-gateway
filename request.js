...
module.exports = async(load, client_key)=> {
  if(!client_key) {
    log.info(CAT.MISSING_KEY,"DENIED","API key was not supplied",`${load.protocol}/${load.method}/${load.host}/${load.port}/${load.path}`);
    return {status:false,error:'Border Gateway API key is not supplied '}
  }

  const key  = verify(client_key)
  if(!key.valid){
    log.info(CAT.INVALID_KEY,"DENIED",key.error?key.error:key.user_id,"API key faild signature matching");
  ...
  try {
    ...
    log.info(CAT.WRONG_AUTH_SERVER_RES,"DENIED",key.user_id,"This could be due to auth server being offline or failing");
