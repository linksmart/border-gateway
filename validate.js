...
const {mqttAuth, AAA, CAT} = require('../iot-bgw-auth-client')

module.exports = async (packet, key)=> {
  let result = true
  switch(  packet.cmd ){
      case 'connect':
        result = await mqttAuth(key,'CON')
        return {
          status : result,
          packet: result?packet:connack()
        }
      case "subscribe":
        result = await packet.subscriptions.reduce(async(t,item)=>t&&(await mqttAuth(key,'SUB',item.topic)),true)
        return {
          status : result,
          packet: result?packet:suback(packet.messageId,new Array(packet.subscriptions.length).fill(128))
        }
      case 'publish':
        result = await mqttAuth(key,'PUB',packet.topic)
