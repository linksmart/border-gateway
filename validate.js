...
module.exports = async(path, client_key,port)=> {
  ...
  if(!key.valid){
    AAA.log(CAT.INVALID_KEY,"DENIED",key.error?key.error:key.user_id,"API key faild signature matching",path,port);
  ...
  if(!profile || !profile.password ||  isNaN(profile.valid_from || NaN) || isNaN(profile.valid_to || NaN)  || !Array.isArray(profile.rules)){
    AAA.log(CAT.PROFILE,"DENIED",key.user_id,"User profile has been removed or corrupted",path,port);
    return {status:false,error:'Supplied BGW API key associated with a user profile that has been removed or corrupted'}
  }
  if(profile.suspended){
    AAA.log(CAT.SUSPENDED,"DENIED",key.user_id,"API key belongs to suspended account",path,port);
  ...
  if(!(profile.valid_from < profile.valid_to &&  now > profile.valid_from && now < profile.valid_to)){
    AAA.log(CAT.EXPIRED,"DENIED",key.user_id,"bgw api key is expired or not valid yet",path,port);
  ...
  if(!correctPassord){
    AAA.log(CAT.PASSWORD,"DENIED",key.user_id,"Wong password, API key has been revoked/renewd",path,port);
