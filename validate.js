...
module.exports = async (packet, key)=> {
  ...
  switch(  packet.cmd ){
      ...
      case "subscribe":
        result = await packet.subscriptions.reduce( async(t,item)=>t&&(await auth(key,'subscribe',item.topic,item.qos)),true)
        return {
          status : result,
          packet: result?packet:suback(packet.messageId,new Array(packet.subscriptions.length).fill(128))
        }
      case 'publish':
        result = await auth(key,'publish',packet.topic,packet.qos)
